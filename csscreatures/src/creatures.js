...
function randomAnimationTiming(target_seconds) {
	var duration = Math.floor((target_seconds/2 + (Math.random() * target_seconds)) * 100) / 100;
	var delay = Math.floor((target_seconds/2 + (Math.random() * target_seconds)) * 100) / 100;
...
function getColor(string) {
  ...
  if (hex) { var color = hex[0]; }
	if (color_name) { var color = color_name[0]; }
...
function getTweets() {
	...
	request.onload = function() {
		...
		if (status == 420 || status == 429) {
			defaultTweets('Too busy!', 'We\'re sorry, there\'s a lot of people visiting right now and we can\'t keep up. Please come back another time.');
		}

		// Twitter is not working
		if (status == 500 || status == 502) {
			defaultTweets('Twitter is down!', 'It looks like Twitter isn\'t working right now. You\'ll have to come back another time.');
		}
	};

	request.onerror = function() {
		defaultTweets('Oh no!', 'Something isn\'t working. Please com eback another time.');
	};
...
function defaultTweets(message_headline, message_body) {
	var message = '<div class="item default-box creature-box mod-tooth mod-sad"><div class="creature" style="background-color: tomato; animation-duration: 5.33s; animation-delay: -6.11s"><div class="face" style="animation-duration: 6s; animation-delay: -7s"><div class="eyes"><div class="eye" style="animation-duration: 6s; animation-delay: -5s"></div><div class="eye" style="animation-duration: 6s; animation-delay: -5s"></div></div><div class="mouth"><div class="tooth"></div></div></div></div><div class="info"><h2>' + message_headline + '</h2><h3>' + message_body + '</h3></div></div>'
...
function buildCreature(tweet, i) {
	...
	for (var n = 0; n < mod_array.length; n++) {
		var regex = RegExp('\\b' + mod_array[n].mod + '|'  + mod_array[n].syn + '\\b');
...
function handleDirectionsToggle() {
	...
	for (var i = 0; i < directions_toggle.length; i++) {
		directions_toggle[i].addEventListener('click', function() {
			...
			document.querySelector('html').classList.toggle('showing-directions');
	...
	creatures.addEventListener('click', function() {
		if (document.querySelector('html').classList.contains('showing-directions')) {
			document.querySelector('html').classList.remove('showing-directions');

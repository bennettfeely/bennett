...
function prettyGradientize(colors, fallback) {
	...
	css =
		'<span class="prop">background</span>: <span class="color">' +
		fallback +
		"</span>;\n";
	css +=
	...
	$.each(colors, function (i, color) {
		...
		if (i == 9) {
			...
			scss_array +=
				...
				"-fallback</span>;\n";
			scss_array +=
				'<span class="prop">background-image</span>: <span class="function">linear-gradient</span>(<span class="deg">90deg</span>, <span class="variable">$' +
				name +
				"-array</span>);";
		}
	});
...
function jsArrayIze(name, colors, fallback) {
	...
	return (
		'<span class="var">var</span> <span class="variable">' +
		name +
		'_fallback</span> = <span class="value">\'' +
		fallback +
		'\'</span>;\n<span class="var">var</span> <span class="variable">' +
		name +
		"_array</span> = [" +
...
function init() {
	...
	$.each(gradients, function (i, gradient) {
		...
		var fig =
			...
			gradientize(colors, fallback) +
			'"></div>' +
			'<div class="gradient-output container">' +
			'<div class="gradient-code css on"><pre class="container">' +
			prettyGradientize(colors, fallback) +
			"</pre></div>" +
			'<div class="gradient-code scss"><pre class="container">' +
			scssIze(gradient.name, colors, fallback) +
			"</pre></div>" +
			'<div class="gradient-code scss-array"><pre class="container">' +
			scssArrayIze(gradient.name, colors, fallback) +
			"</pre></div>" +
			'<div class="gradient-code js-array"><pre class="container">' +
			jsArrayIze(gradient.name, colors, fallback) +
			"</pre></div>" +
			'<div class="gradient-code list"><pre class="container">' +
			listIze(colors, fallback) +
			"</pre></div>" +
			'<a class="copy" data-parent="' +
			gradient.name +
			'" data-clipboard-target=".card.' +
			gradient.name +
			' .gradient-code.on">Copy to Clipboard</a>' +
			"</div>" +
			"</div>" +
			"</figure>" +
			"</div>";
...
function clipBoardize() {
	...
	$(".gradient-code.on").mousedown(function () {
		...
		$("." + parent).mouseleave(function () {
			setTimeout(function () {
				$(".copy.is-shown")
					.removeClass("is-shown is-copied")
...
function exportCodePen() {
	...
	$(".codepen").click(function () {
		...
		var DESCRIPTION =
			'"' +
			gradient_name +
			'" gradient in CSS. Generated from http://bennettfeely.com/csscales.';
		var EDITORS = "";
		...
		if (curr_lang == "scss") {
			...
			var CSS = gradient_scss
				.replace(/background:/g, "body {\n\tbackground:")
